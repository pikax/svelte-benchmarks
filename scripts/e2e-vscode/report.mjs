 ...
 *     nothing at all while the row still carries a `primaryMs`. Timing a null
 *     answer measures how quickly a server can decline, not hover latency.
 *
 ...
 *     TypeScript type, and the Svelte markup expression must also carry a typed
 *     answer. Returning only an untyped symbol at either position misses part
 *     of the requested language-service work.
 *
...
export const METHODOLOGY = [
  ...
  "Workspaces: regular (single app), monorepo (shared package + app).",
  ...
  "**Hover content is gated, and a row must pass to be ranked.** Probe: `const benchMarker: string` in `<script lang=\"ts\">` and `{benchMarker}` in markup. Both positions are checked for a typed answer.",
  "Rows that fail the gate, answer nothing, or sit at a position with no known-correct answer are **measured but unranked**: the time is shown in brackets with the reason.",
  "Activation is reported only when this run performed it. An extension already active when the subject was resolved shows `n/a (pre-activated)`.",
  "Diagnostics wait is measured from a COMMON origin for every subject: the subscription is registered and the clock started BEFORE the document is opened.",
  "Marketplace defaults: svelte.svelte-vscode, baseballyama.rsvelte-vscode, verter.verter-vscode.",
];
...
export function renderMarkdown(rows) {
  ...
  lines.push("");
  lines.push(
    "Setup: same VS Code stable build; isolated `--extensions-dir` per subject; only that Svelte extension installed.",
  );
  lines.push("");
