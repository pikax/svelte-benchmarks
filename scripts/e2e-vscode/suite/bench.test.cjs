 ...
 * Measures extension-host language features for one enabled Svelte extension.
 *
 * Env (set by run.mjs):
 *   E2E_WORKSPACE   absolute workspace path
 *   E2E_EXTENSION   marketplace id (Svelte.svelte-vscode | baseballyama.rsvelte-vscode | verter.verter-vscode)
...
suite("VS Code E2E Svelte language extension", function () {
  ...
  test("open + hover + completion latency", async () => {
    ...
    if (extensionId) {
      ...
      if (!ext) {
        const svelteish = vscode.extensions.all
          .filter((e) => /svelte|rsvelte|verter/i.test(e.id))
          .map((e) => e.id);
        throw new Error(
          `Extension not found: ${extensionId}. Svelte-related loaded: ${svelteish.join(", ") || "(none)"}`,
        );
      }
    ...
    // position (markup expression) so the Svelte language service is exercised.
