 ...
 *   2. Launch headless with only that Svelte extension enabled
 ...
 *   - Same Svelte workspaces (regular / monorepo)
 ...
 *   svelte.svelte-vscode         Official Svelte for VS Code
 *   baseballyama.rsvelte-vscode  rsvelte (when published)
 *   verter.verter-vscode         Verter
 *
 * Override with --extensions id=Label,... or env SVELTE_EXTENSION_ID / VERTER_EXTENSION_ID.
...
// Published marketplace IDs (ranking is by measured metric).
// rsvelte-vscode is optional — skipped install failures still write an error row.
const DEFAULT_EXTENSIONS = [
  { id: "svelte.svelte-vscode", label: "Svelte" },
  { id: "baseballyama.rsvelte-vscode", label: "rsvelte", optional: true },
...
function parseArgs(argv) {
  const args = {
    ...
    svelteExtension: process.env.SVELTE_EXTENSION_ID || "svelte.svelte-vscode",
    rsvelteExtension:
      process.env.RSVELTE_EXTENSION_ID || "baseballyama.rsvelte-vscode",
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === "--svelte-extension") args.svelteExtension = argv[++i];
    else if (a === "--rsvelte-extension") args.rsvelteExtension = argv[++i];
...
function buildExtensionList(args) {
  ...
  const list = [
    { id: args.svelteExtension, label: "Svelte" },
    { id: args.rsvelteExtension, label: "rsvelte", optional: true },
  ];
  ...
  if (args.verterVsix) {
    ...
    list.push({ id: args.verterExtension, label: "Verter" });
  }
  return list;
}
...
function ensureWorkspaces(args) {
  ...
  const r = spawnSync(process.execPath, [join(__dirname, "setup-workspaces.mjs")], {
    cwd: rootDir,
    stdio: "inherit",
  });
...
function listInstalledExtensionDirs(extensionsRoot, extensionId) {
  ...
  // Marketplace folder names are lowercased (publisher.name-version).
...
function ensureSubjectExtension({ vscodeExecutablePath, extDir, extension }) {
  const userDataDir = join(
    os.tmpdir(),
    `svelte-bench-e2e-userdata-${extension.label.replace(/\W+/g, "_")}-${process.pid}`,
  );
 ...
 * Override only through E2E_LAUNCH_TIMEOUT_MS, keeping the job ceiling above
 * the worst-case sum of sequential launch budgets.
 *
...
async function runOne({ workspaceName, extension, resultsDir }) {
  ...
  // Isolate extensions dir per subject so only one Svelte LS is active
  const extDir = join(
    os.tmpdir(),
    `svelte-bench-e2e-exts-${extension.label.replace(/\W+/g, "_")}`,
  );
...
Options:
  ...
  --svelte-extension  <id>           (default: svelte.svelte-vscode)
  --rsvelte-extension <id>           (default: baseballyama.rsvelte-vscode)
