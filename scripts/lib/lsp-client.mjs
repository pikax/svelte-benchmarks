...
export class LspClient extends EventEmitter {
  ...
  constructor(name, command, args = [], options = {}) {
    ...
    this.#proc.on("error", (err) => {
      ...
      for (const [id, pending] of this.#pending) {
        ...
        pending.reject(
          new Error(`${this.#name}: server process failed: ${err.message}`),
        );
      }
     ...
     *
     * Kept because a server can answer requests while reporting a degraded or
     * failed backend only on stderr. That distinction may not appear in LSP
     * traffic, so the bounded tail remains available to surface-specific gates.
     */
    this.stderrTail = "";
  ...
  /**
   * Resolve a dotted configuration section ("svelte.plugin.svelte", "css.customData")
   ...
   * read a section expecting an array or scalar and receive an object instead.
   * `null` lets the server use its own default.
   */
  ...
  sendRequest(method, params, timeoutMs = 30_000) {
    if (this.#procError) {
      return Promise.reject(
        new Error(
          `${this.#name}: server process failed: ${this.#procError.message}`,
        ),
      );
    }
  ...
  waitForNotification(methods, timeoutMs = 60_000) {
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(
          new Error(
            `${this.#name}: timeout waiting for ${[...set].join("|")} (${timeoutMs}ms)`,
          ),
        );
      }, timeoutMs);
  ...
  async initialize(
    rootUri,
    { initializationOptions, timeoutMs = 30_000 } = {},
  ) {
