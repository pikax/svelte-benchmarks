...
export async function runCompileSurface(fixtureDir, options) {
  ...
  return {
    ...
    methodology: [
      ...
      "Source-map dimension: deliberately NOT benchmarked. Every compared Svelte 5 compiler (official 5.56.8/5.56.4, @mrwaip/svelte-rs, @rsvelte/*) ALWAYS emits js.map/css.map from compile() — there is no compiler-level off/on flag (the 'sourcemap' option is a chained-map INPUT; passing true crashes TraceMap inside the official compiler). An off/on matrix would therefore measure the harness's serialization choice, not the tools. A future mapping-CORRECTNESS oracle (planted positions traced back to source coordinates) would be a genuinely new dimension.",
